class ApplicationController < ActionController::Base
  ...
  def setup_widgets
    ...
    if Rails.env.production?
      @recent_pomodoros = Pomodoro.successful_and_completed.select("activity_id, *").group("activity_id, pomodoros.id, created_at").order("pomodoros.created_at DESC").limit(5)
    else
